...
def _rotate(path: Path, retention: int = 5):
    ...
    try:
        ...
        candidates = sorted([p for p in path.parent.iterdir() if p.is_file() and re.fullmatch(pattern, p.name)], 
...
def configure_logging(
    ...
    def resolve_level(arg, env, key, default):
    ...
    target_name = discovery.determine_script_name(script_name or os.getenv("LOGFLOW_SCRIPT_NAME") or file_cfg.get("script_name"))
    new_log_file = log_dir_path / f"{target_name}.log"

    # 3. PIVOT & ROTATION (Authority: Main Process Only)
    if is_main_proc:
        current_abs = _STATE["log_file"].resolve() if _STATE["log_file"] else None
        ...
        if current_abs and new_abs != current_abs:
            ...
            if _STATE["log_file"] and _STATE["log_file"].exists():
                try:
                    shutil.copy2(_STATE["log_file"], new_log_file)
                    time.sleep(0.05) # Release OS Lock
                    _STATE["log_file"].unlink()
                except Exception: pass
            _STATE["configured"] = False
    ...
    if is_main_proc:
        ...
        time.sleep(0.05)
        lfs = sorted([f for f in log_dir_path.glob("*.log") if f.is_file()], 
