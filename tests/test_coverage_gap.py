...
import shutil
import time
from logflow.core import configure_logging, shutdown_logging, _STATE
import logflow.discovery
from loguru import logger

def test_rotate_non_zero_rank(tmp_path: Path, monkeypatch):
...
def test_rotate_cleanup_error(tmp_path: Path, monkeypatch):
...
def test_configure_already_configured_no_force():
...
def test_pivot_copy_error(tmp_path: Path, monkeypatch):
...
def test_pivot_unlink_error(tmp_path: Path, monkeypatch):
...
def test_global_retention_unlink_error(tmp_path: Path, monkeypatch):
...
def test_shutdown_logging_exception(monkeypatch):
...
def test_pivot_copy_generic_exception(tmp_path: Path, monkeypatch):
...
def test_rotate_rename_generic_exception(tmp_path: Path, monkeypatch):
...
def test_pivot_complete_exception(tmp_path: Path, monkeypatch):
